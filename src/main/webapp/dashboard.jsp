...
<%
    UserDataClass usuario = (UserDataClass) session.getAttribute("usuario");
    if ( usuario != null ){
%>
                        ...
                        <div class="modal-body">
                            ...
                            <form class="formAddTask" id="IdFormAddTask" action="#" novalidate>
                                ...
                                <div class="form-group form-row justify-content-center">
                                    <div class="col-auto " >
                                        <label>Date </label>
                                        <input type="date" class="form-control" name="date" min="<%= LocalDate.now().toString()%>" required >
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    ...
                    <form  id="formDate" method="POST" action="datetasks.jsp" >
                        <div class="form-group px-2">
                            <label>
                                <input class="form-control" type="date" name="dateFilter" value="<%= LocalDate.now().toString()%>" required>
                            </label>
                            <hr>
                        </div>
                    </form>
...
<%-- Show out the name of user --%>
                            <% out.println( usuario.getName()+" "+usuario.getSurname() );%>
                            </span>
...
<script>
    $(document).ready(function () {
        $("#deletebtn").click(function() {
            var combinedData = $("#formtask").serialize();
    ...
    function todayTasks(){
        $(document).ready( function (){
            $('#tasksGroup').remove(); // you can keep this line if you think is necessary
            $('#showTasks').load("todaytasks.jsp");
            })
    }
    function monthTasks(){
        $(document).ready( function (){
            $('#tasksGroup').remove(); // you can keep this line if you think is necessary
            $('#showTasks').load("monthtasks.jsp");
        })
    }

    function dateTask( Id ){
        $(document).ready( function (){
            var combinedData = $("#" + Id).serialize();
            console.log(combinedData);
            $.ajax({
               ...
               /* success: function (result) {
                    alert(result);
                    /!*window.location.href = "datetasks.jsp";
                    $('#tasksGroup').remove(); // you can keep this line if you think is necessary
                    $('#showTasks').load("datetasks.jsp");*!/
                },*/
                error: function (xhr, ajaxOptions, thrownError, exception){
                    ...
                    /*alert(response)*/
                    console.log("aqui estoy corriendo")
                    $('#tasksGroup').remove(); // you can keep this line if you think is necessary
    ...
    $("#formterm").submit( function (event) {
        ...
        var combinedData = $("#formterm").serialize();
        console.log(combinedData);
        $.ajax({
            type: "POST",
            url: "term.jsp",
    ...
    /* test de probar la subida por ajax y el formulario*/
    function TasksByMonth( num ){
        $.ajax({
            type: "POST",
            url: "monthfiltertasks.jsp",
            data: { monthFilter : num },
            error: function (xhr, ajaxOptions, thrownError, exception){
                console.log('ERROR: jqXHR, exception', exception, xhr, ajaxOptions, thrownError );
            }
    ...
    $(".monthCheck").click( function (event) {
        var combinedData = $("#formGroupMonth").serialize();
    ...
    function dateTasktes(){
            $(document).ready( function (){
                var getDateTasks = $("#formdate").serialize();
                console.log(getDateTasks);
                $.ajax({
                    type: "POST",
                    dataType: 'text/html',
                    url: "borrartest.jsp",
                    data: getDateTasks,
                    complete: function (){
                        window.location.href = "borrartest.jsp";
                        /*$('#tasksGroup').remove(); // you can keep this line if you think is necessary
                        $('#showTasks').load("datetasks.jsp");*/}
    ...
    $( "#formDate" ).submit(function( event ) {
        ...
        var $form = $( this ),
            term = $form.find( "input[name='dateFilter']" ).val(),
            url = $form.attr( "action" );

        // Send the data using post
        console.log(term);
        var posting = $.post( url, { dateFilter: term } );
