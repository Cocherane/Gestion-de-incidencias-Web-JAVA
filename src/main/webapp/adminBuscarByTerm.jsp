...
<%
  ...
  if ( session.getAttribute("user") != null || session.getAttribute("user") instanceof Admin){
    ...
    ArrayList<IncidenciaDataClass> incidenciasData = (ArrayList<IncidenciaDataClass>) session.getAttribute("incidenciaByTerm");
    // get all Tecnicos
    ArrayList<TecnicoDataClass> tecnicos = app.getTecnicos();

    // check if Arraylist<IncidenciaDataClass> is no empty
    if ( !incidenciasData.isEmpty() ){
%>
<%-- titulo de las incidencias --%>
<div class="col-lg-12 pb-3 pb-md-4">
  <h1 class="h5 fst-italic mb-0 text-gray-600 fw-bolder text-capitalize">Incidencias por el termino buscado</h1>
</div>
...
<%
  ...
  int indexTecnico = 1;
  for (IncidenciaDataClass incidencia : incidenciasData) {
    counterSetOffcanvas++;
    ...
    <div class="card-body text-light">
      <ul class="list-group list-group-flush">
        ...
        <li class="list-group-item ">Estado :
          <div class="progress">
            <div class="progress-bar <%= incidencia.isEstaResuelta()?"bg-success":(incidencia.getIdTecnico() == 0 )? "bg-danger" : "bg-primary "%> bg-primary progress-bar-striped progress-bar-animated text-center" role="progressbar" aria-label="Animated striped example" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100" style="width: <%= incidencia.isEstaResuelta()? 100 :(incidencia.getIdTecnico() == 0 )? 33 : 66 %>%">
              <%= incidencia.isEstaResuelta()?"RESUELTA": (incidencia.getIdTecnico() == 0 )?"POR ASIGNAR": "ASIGNADA"%>
            </div>
          </div>
        </li>
      </ul>
      <div class="d-flex justify-content-center mx-auto py-1">
        ...
        <%-- si no esta resuelta presento el boton para mostrar para asignar o modificar--%>
        <%
        ...
        %>
          <%-- END si no esta resuelta presento el boton para mostrar para asignar o modificar--%>
      ...
      <div class="modal-body">
        <div class="card-body text-light">
          <ul class="list-group list-group-flush">
            ...
            <%-- just if the incident is resuelt --%>
            <%
              if ( incidencia.isEstaResuelta() ){
            %>
            ...
            %>
            <%-- END just if the incident is resuelt --%>
            ...
            <td>
              <form id="form-user-set-incidenia-tecnico-<%=indexTecnico%>" class="text-center"  >
                <div class="btn-group ">
                  ...
                  <input type="hidden" value="3" name="idType">
